...
export class GameManager extends Component {
    ...
    public createEnemyBullert(targetPos: Vec3) {
        //const bullet = instantiate(this.bullet01);//实例化
        const bullet = PoolManager.instance().getNode(this.bullet02, this.bulletRoot);//实例化   改变敌方子弹外观
        // bullet.setParent(this.bulletRoot);//将所有子弹放在该节点下
        const pos = this.playerPlane.node.position;//记录飞机位置
    ...
    public creteCombination2() {
        ...
        for (let i = 0; i < enemyArray.length; i++) {
            ...
            enemyArray[i] = PoolManager.instance().getNode(this.enemy02, this.node);//实例化   改变敌机子弹外形
            const element = enemyArray[i];
            // element.parent = this.node;
    ...
    public creteCombination3() {
        ...
        const boss = PoolManager.instance().getNode(this.Boss, this.node);//实例化   改变敌机子弹外形

        // boss.parent = this.node;
        boss.setPosition(0.8, 0, -34);//
