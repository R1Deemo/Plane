...
//import { SelfPlane } from './SelfPlane';
...
export class Boss extends Component {
    ...
    public bossLifeValue = 100;//设置boss最高血量
    public bossIsLive = false;//判断存活状态

    private _bossCurrLife = 0;//boss当前血量
    ...
    update(deltaTime: number) {
        ...
        // const movePos = pos.z + 1;
        //生成子弹
        if (this._needBullet && !this.bossIsLive) {
            this._currCreateBulletTime += deltaTime;
            if (this._currCreateBulletTime > this.createBulletTime) {
                this._currCreateBulletTime = 0;
                this._gameManage.createBossBullert(this.node.position)

            }
        }
        if (this._gameManage.playerPlane._currLife == 0) {
            this.bossIsLive = false;
            PoolManager.instance().putNode(this.node);
            this._gameManage.gameOver();
        }
    ...
    private _onTriggerEnter(event: ITriggerEvent) {
        ...
        if (collisionGroup === Constant.CollisionType.SELF_PLANE || Constant.CollisionType.SELF_BULLET) {
            ...
            // console.log(this._gameManage.playerPlane.isDie);//false
            ...
            // this.bloodface.setScale(this._bossCurrLife / this.bossLifeValue, 1, 1);//设置血条
        ...
        // console.log(this._gameManage.playerPlane._currLife)
